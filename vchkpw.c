...
int main(int argc, char **argv) {
  ...
  if ((tmpstr = getenv("TCPLOCALPORT")) == NULL)
    LocalPort = 0;
  else
  ...
  switch (LocalPort) {
    ...
    default:
       ...
       */
      if (strstr(argv[1] ? argv[1] : "", "true") !=
          NULL) /* used as STMP AUTH */
        ConnType = SMTP_CONN;
      else if (strstr(argv[1] ? argv[1] : "", "imap") !=
               NULL) /* used with IMAP */
        ConnType = IMAP_CONN;
      else /* default to POP */
        ConnType = POP_CONN;
  ...
  if (parse_email(TheName, TheUser, TheDomain, AUTH_SIZE) != 0) {
    snprintf(LogLine, sizeof(LogLine),
             "%s: invalid user/domain characters %s:%s", VchkpwLogName, TheName,
             IpAddr);
  ...
  if (ConnType != SMTP_CONN && setgid(pw_gid) == -1) {
    snprintf(LogLine, sizeof(LogLine),
             "%s: setgid %lu failed errno %d %s@%s:%s", VchkpwLogName,
             (long unsigned)pw_gid, errno, TheUser, TheDomain, IpAddr);
    vlog(VLOG_ERROR_INTERNAL, TheUser, TheDomain, ThePass, TheName, IpAddr,
         LogLine);
    vchkpw_exit(4);
  }
  ...
  if (ConnType != SMTP_CONN && setuid(pw_uid) == -1) {
    snprintf(LogLine, sizeof(LogLine),
             "%s: setuid %lu failed errno %d %s@%s:%s", VchkpwLogName,
  ...
  if (ConnType != SMTP_CONN && chdir(pw_dir) == -1) {
    if (vpw != NULL) {
      if (vmake_maildir(TheDomain, vpw->pw_dir) != VA_SUCCESS) {
        snprintf(LogLine, sizeof(LogLine),
                 "%s: autocreate dir errno %d %s %s@%s:%s", VchkpwLogName,
      ...
      snprintf(LogLine, sizeof(LogLine),
               "%s: chdir failed errno %d %s %s@%s:%s", VchkpwLogName, errno,
               pw_dir, TheUser, TheDomain, IpAddr);
      vlog(VLOG_ERROR_INTERNAL, TheUser, TheDomain, ThePass, TheName, IpAddr,
           LogLine);
      vchkpw_exit(6);
    }
  ...
  if (putenv(envbuf1) == -1) {
    snprintf(LogLine, sizeof(LogLine),
             "%s: putenv(USER) failed errno %d %s@%s:%s", VchkpwLogName, errno,
  ...
  if (putenv(envbuf2) == -1) {
    snprintf(LogLine, sizeof(LogLine),
             "%s: putenv(HOME) failed errno %d %s@%s:%s", VchkpwLogName, errno,
             TheUser, TheDomain, IpAddr);
    vlog(VLOG_ERROR_INTERNAL, TheUser, TheDomain, ThePass, TheName, IpAddr,
         LogLine);
    vchkpw_exit(8);
  ...
  if (putenv(envbuf3) == -1) {
    snprintf(LogLine, sizeof(LogLine),
             "%s: putenv(SHELL) failed errno %d %s@%s:%s", VchkpwLogName, errno,
             TheUser, TheDomain, IpAddr);
    vlog(VLOG_ERROR_INTERNAL, TheUser, TheDomain, ThePass, TheName, IpAddr,
         LogLine);
    vchkpw_exit(9);
  ...
  if (putenv(envbuf4) == -1) {
    snprintf(LogLine, sizeof(LogLine),
             "%s: putenv(VPOPUSER) failed errno %d %s@%s:%s", VchkpwLogName,
...
void read_user_pass() {
  ...
  for (i = 0; i < AUTH_SIZE; i += j) {
    ...
    if (j == -1) {
      fprintf(
          stderr,
          "%s: vchkpw is only for talking with qmail-popup and qmail-pop3d. \
...
void login_virtual_user() {
  ...
  if (vpw->pw_dir == NULL || vpw->pw_dir[0] == 0) {
    ...
    if ((vpw = vauth_getpw(TheUser, TheDomain)) == NULL) {
      snprintf(LogLine, sizeof(LogLine),
               "%s: failed to vauth_getpw() after dir auto create %s@%s:%s",
               VchkpwLogName, TheUser, TheDomain, IpAddr);
      vlog(VLOG_ERROR_INTERNAL, TheUser, TheDomain, ThePass, TheName, IpAddr,
           LogLine);
      vchkpw_exit(14);
  ...
  if ((cramaccepted != 0) && (apopaccepted != 0) &&
    ...
    if (ENABLE_LOGGING == 1 || ENABLE_LOGGING == 2) {
      ...
      snprintf(LogLine, sizeof(LogLine),
               "%s: password fail (pass: '%s') %s@%s:%s", VchkpwLogName,
  ...
  if (ConnType == POP_CONN && (vpw->pw_flags & NO_POP)) {
    snprintf(LogLine, sizeof(LogLine), "%s: pop access denied %s@%s:%s",
             VchkpwLogName, TheUser, TheDomain, IpAddr);
    vlog(VLOG_ERROR_ACCESS, TheUser, TheDomain, ThePass, TheName, IpAddr,
         LogLine);
    vchkpw_exit(1);
  }
...
void login_system_user() {
  ...
  if ((spw = getspnam(TheUser)) == NULL) {
    snprintf(LogLine, sizeof(LogLine),
             "%s: system user shadow entry not found %s:%s", VchkpwLogName,
             TheName, IpAddr);
    vlog(VLOG_ERROR_LOGON, TheUser, TheDomain, ThePass, TheName, IpAddr,
         LogLine);
    vchkpw_exit(22);
  ...
  if (strcmp(c, pw->pw_passwd) != 0) {
    ...
    if (ENABLE_LOGGING == 1 || ENABLE_LOGGING == 2) {
      ...
      snprintf(LogLine, sizeof(LogLine),
               "%s: system password fail (pass: '%s') %s:%s", VchkpwLogName,
  ...
  if (ENABLE_LOGGING == 1 || ENABLE_LOGGING == 4) {
    snprintf(LogLine, sizeof(LogLine),
             "%s: system password login success %s:%s", VchkpwLogName, TheUser,
             IpAddr);
    vlog(VLOG_AUTH, TheUser, TheDomain, ThePass, TheName, IpAddr, LogLine);
  }
